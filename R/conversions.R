...
#'
#' @export
ci.se <- function (est, cil, ciu, n, k=0, result = c("ciu", "cil", "avg", "diff")) {
...
#' mean and sd to d
#'
#' means and standard deviations of two groups to cohen's d\cr
#' updated equation: spooled = sqrt((sd1^2*(n1-1) + sd2^2*(n2-1))/(n1+n2-2))\cr
#'
...
meansd.d <- function(m1, m2, sd1, sd2, n1, n2) {
  ...
  spool_num = sd1^2*(n1-1) + sd2^2*(n2-1)
  spool_den = n1+n2-2
  spool = sqrt(spool_num/spool_den)
...
#' mean and sd to g
#'
#' means and standard deviations of two groups to hedges' g
#' updated equation: spooled = sqrt((sd1^2*(n1-1) + sd2^2*(n2-1))/(n1+n2-2))\cr
#'
#' @param m1 mean of group 1
#' @param m2 mean of group 2
...
#'
#' @examples
#' meansd.g(15.1, 13.2, 2.3, 1.1, 25, 45)
#' dat %>% mutate (g_from_meansd = meansd.g(stress_mean, control_mean, stress_sd, control_sd, stress_n, control_n)) -> dat
#'
#' @export
meansd.g <- function (m1, m2, sd1, sd2, n1, n2) {
...
#' mean and se to g
#'
#' means and standard errors of two groups to cohen's g
#'
#' @param m1 mean of group 1
#' @param m2 mean of group 2
#' @param se1 standard error of group 1
...
#'
#' @examples
#' meanse.g(15.1, 13.2, .64, .164, 25, 45)
#'
#' @export
meanse.g <- function (m1, m2, se1, se2, n1, n2) {
  d = meanse.d(m1, m2, se1, se2, n1, n2)
  g = d.g(d, n1+n2)
  return (g)
}
...
#' conversion is based on boreinstein et al., 2009 introduction to meta-analysis, equation 7.8
#'
#' @param d cohen's d
...
meanci.d <- function (m1, m2, cil1, ciu1, cil2, ciu2, n1, n2, k=1) {
  ...
  sd1 = se.sd(se1_ciu, n1)
  sd2 = se.sd(se2_ciu, n2)
...
#' mean and ci to g
#'
#' means and confidence interval of two groups to hedges' g
#'
#' @param m1 mean of group 1
#' @param m2 mean of group 2
#' @param cil1 lower bound of 95 confidence interval
...
#'
#' @export
meanci.r <- function (m1, m2, cil1, ciu1, cil2, ciu2, n1, n2, k=0, result = c("sediff", "cil", "ciu")){
  # if (missing(k)) {
...
#' delta r squared to r
#' 
#' note that this is only correct if the delta r squared of the focal predictor (not the omnibus model) is provided
#'
#' @param r2 delta r squared for the focal predictor (not the omnibus model)
#' @examples r2.r(r2)
#' @examples dat %>% mutate (r2_to_r = r2.r(r2))-> dat
#'
#' @export
r2.r <- function (r2) {
  r<-sqrt(r2)
...
#'
#' @export
geoci.se <- function (geomean, cil, ciu, n, k=0, result = c("cil", "ciu")) {
  # if (missing(k)) {
  #   stop ('hi, please specify k (the number of predictors); for raw means without covariates, enter k = 0')
  # } #k = 0 would give df = n - 1 for each mean
...
#'
#' @export
geo.r <- function (m1, m2, cil1, ciu1, cil2, ciu2, n1, n2, k=0, result = c("sediff", "cil", "ciu")){
  # if (missing(k)) {
...
#'
#' @export
geo.d <- function (m1, m2, cil1, ciu1, cil2, ciu2, n1, n2, k=0, result = c("sediff", "cil", "ciu")){
  # if (missing(k)) {
  #   stop ('hi, please specify k-- the number of covariates these means are adjusted for, enter k = 0 if unadjusted.')
  # } #k = 0 would give df = n - 1 for each mean
